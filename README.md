...
The objective of this project is to build and compare multiple machine learning classification models to predict whether a loan applicant is a good or bad credit risk based on financial and demographic attributes.

The task is a binary classification problem using the UCI Statlog (German Credit Data) dataset. The goal is to evaluate model performance using multiple evaluation metrics and determine the best-performing model.
...
- Target Variable:
  - 1 → Good Credit Risk
  - 2 → Bad Credit Risk (converted to binary format)
...
Among all implemented models, **Random Forest** achieved the best overall performance across most evaluation metrics, including Accuracy, F1 Score, and MCC. Ensemble methods outperformed individual models, indicating that combining multiple weak learners improves predictive capability for credit risk classification.
